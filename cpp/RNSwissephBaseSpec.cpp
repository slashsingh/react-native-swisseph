...
namespace facebook
{
  namespace react
  {
    ...
    static facebook::jsi::Value __hostFunction_NativeSwissephSpecJSI_sweTimeEqu(facebook::jsi::Runtime &rt, TurboModule &turboModule, const facebook::jsi::Value *args, size_t count)
    {
      try
      {
        double tjd = args[0].getNumber();
        double result = swisseph::swe_time_equ(tjd);
        return facebook::jsi::Value(result);
      }
      catch (std::exception &e)
      {
    ...
    static facebook::jsi::Value __hostFunction_NativeSwissephSpecJSI_sweRiseTrans(facebook::jsi::Runtime &rt, TurboModule &turboModule, const facebook::jsi::Value *args, size_t count)
    {
      try
      {
        double tjd_ut = args[0].getNumber();
        int ipl = args[1].getNumber();
        const char *starname = args[2].isNull() || args[2].isUndefined() ? nullptr : args[2].getString(rt).utf8(rt).c_str();
        int epheflag = args[3].getNumber();
        int rsmi = args[4].getNumber();
        double atpress = args[5].getNumber();
        double attemp = args[6].getNumber();

        auto result = swisseph::swe_rise_trans(
            tjd_ut,
            ipl,
            starname,
            epheflag,
    ...
    std::unordered_map<std::string, SwissephMethodMetadata> createMethodMap()
    {
      return {
          ...
          {"sweTimeEqu", SwissephMethodMetadata{1, __hostFunction_NativeSwissephSpecJSI_sweTimeEqu}},
          {"sweRiseTrans", SwissephMethodMetadata{7, __hostFunction_NativeSwissephSpecJSI_sweRiseTrans}}};
    }
  }
}
