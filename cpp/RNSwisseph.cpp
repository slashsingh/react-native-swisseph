...
std::map<std::string, std::vector<double>> swe_nod_aps_ut(double tjd_ut, int ipl, int iflag, int method) {
  ...
  return {
    {"ascending", std::vector<double>(xnasc, xnasc + 6)},
    {"descending", std::vector<double>(xndsc, xndsc + 6)},
    {"perihelion", std::vector<double>(xperi, xperi + 6)},
    {"aphelion", std::vector<double>(xaphe, xaphe + 6)}
  };
}
...
std::map<std::string, double> swe_fixstar(const std::string &star, double tjd, int iflag) {
  ...
  if (result < 0) throw std::runtime_error(serr);
...
std::vector<double> swe_vis_limit_mag(double tjd_ut, std::vector<double> dgeo, std::vector<double> datm, std::vector<double> dobs, const std::string& object_name, int helflag) {
  ...
  if (result >= 0) {
    ...
    ret.push_back(0); // placeholder
  }
  return ret;
}

double swe_time_equ(double tjd) {
  double E;
  char serr[AS_MAXCH];
  int32 result = ::swe_time_equ(tjd, &E, serr);
  if (result < 0) {
    throw std::runtime_error(serr);
  }
  return E;
...
double swe_rise_trans(double tjd_ut, int ipl, const std::string& starname, int epheflag, int rsmi, double atpress, double attemp) {
  ...
  char serr[AS_MAXCH];
  double geopos[3];
  int result = ::swe_rise_trans(tjd_ut, ipl, const_cast<char*>(starname.c_str()), epheflag, rsmi, geopos, atpress, attemp, &trise, serr);
  if (result < 0) {
    throw std::runtime_error(serr);
  }
  return trise;
